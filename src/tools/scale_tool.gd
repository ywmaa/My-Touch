...
func enable_tool(): # Save History and Enable Tool
	ToolsManager.current_project.project.undo_redo.create_action("Scale Layers")
	for selected in ToolsManager.current_project.project.layers.selected_layers:
		ToolsManager.current_project.project.undo_redo.add_undo_property(selected,"scale",selected.scale)
	direction = coordinates.xy
	super.enable_tool()
func cancel_tool(): # Redo Actions
	if ToolsManager.current_project.project.layers.selected_layers:
		for selected in ToolsManager.current_project.project.layers.selected_layers:
			ToolsManager.current_project.project.undo_redo.add_do_property(selected, "scale", selected.scale)
