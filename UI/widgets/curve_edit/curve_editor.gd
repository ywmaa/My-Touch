extends "res://UI/widgets/curve_edit/curve_view.gd"
...
func update_controls() -> void:
	...
	for i in curve.points.size():
		...
		var control_point = preload("res://UI/widgets/curve_edit/control_point.tscn").instance()
		...
		if i == 0 or i == curve.points.size()-1:
			control_point.set_constraint(control_point.rect_position.x, control_point.rect_position.x, -control_point.OFFSET.y, size.y-control_point.OFFSET.y)
			if i == 0:
