...
#	var current_layer: int = Global.current_project.current_layer
	...
	if mt_globals.mirror_view:
		position_tmp.x = position_tmp.x + project.project.canvas_size.x
		scale_tmp.x = -1
	draw_set_transform(position_tmp, rotation, scale_tmp)
	# Draw current frame layers
	for i in project.project.layers.layers:
		i.draw_image()
...
func _process(_delta):
	queue_redraw()
	get_parent().size = project.project.canvas_size



func camera_zoom() -> void:
	# Set camera zoom based on the sprite size
	var canvas_size = project.project.canvas_size
